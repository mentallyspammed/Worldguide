...
# --- Constants ---
LOG_DIR = "botlogs"
ST_LOUIS_TZ = ZoneInfo("America/Chicago")  # Or your preferred timezone
...
class TradingAnalyzer:
    ...
    def calculate_wma(self, window: int) -> pd.Series:
        ...
        wma_indicator = WMAIndicator(self.df["close"], window=window)
        return wma_indicator.wma()

    def calculate_hma(self, window: int) -> pd.Series:
        """Calculates the Hull Moving Average (HMA)."""
        return calculate_hma(self.df["close"], window)
    ...
    def determine_trend_momentum_adx(self) -> str:
        ...
        if adx > 25:  # Strong trend
            if (
                ...
                return "neutral"  # Strong trend but direction unclear
        else:
    ...
    def analyze(self, current_price: float):
        ...
        self.df["Aroon Indicator"] = aroon_data["Aroon Indicator"]
        ...
        self.logger.info(
            f"{Fore.YELLOW}Aroon Up: {Fore.GREEN}{self.df['Aroon Up'].iloc[-1]:.2f}, Aroon Down: {Fore.RED}{self.df['Aroon Down'].iloc[-1]:.2f}, Aroon Indicator: {Fore.CYAN}{self.df['Aroon Indicator'].iloc[-1]:.2f}"
        )
...
def calculate_hma(series: pd.Series, window: int) -> pd.Series:
    ...
    Args:
        series: Pandas Series containing the data (e.g., closing prices).
        window: The moving average window size.

    Returns:
        Pandas Series containing the HMA values.
    """
    wma_half_length = pd.Series(series).rolling(window=int(window / 2)).mean()
    wma_full_length = pd.Series(series).rolling(window=window).mean()
    subtracted = (wma_half_length * 2) - wma_full_length
