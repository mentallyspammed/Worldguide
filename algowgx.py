...
self.logger.error(f"Bybit API Error: {
    response['retMsg']}")
return None
...
self.logger.error(f"Error fetching current price: {
    e
}")
return None
...
self.logger.warning(f"No kline data found for symbol {
    symbol
}.")
return pd.DataFrame()
...
self.logger.error(f"Error fetching klines: {
    e
}")
return pd.DataFrame()
...
self.logger.error(f"Error fetching order book: {
    e
}")
return {}
...
is_peak = data[i] > max(data[i-window:i]) and data[i] > max(data[i+1:i+1+window])
is_valley = data[i] < min(data[i-window:i]) and data[i] < min(data[i+1:i+1+window])
...
sorted_data = self._insertion_sort(list(data))

clusters = []
cluster = [sorted_data[0]]

for i in range(1, len(sorted_data)):
if np.abs(sorted_data[i] - np.mean(cluster)) < sensitivity*sorted_data[i]: #Adjust sensitivity for larger numbers
...
if not levels:
return {
    "support": [], "resistance": []} # Handle the case with no levels
...
nearest_resistances = sorted(resistances, key = lambda x: x[1] - current_price)[:3]
...
f" {
    ...
    self.df['start_time'].iloc[-1].tz_convert(self.config.timezone).strftime('%Y-%m-%d %H:%M:%S')}"
)
self.logger.info(f" {
    Fore.YELLOW
}Current Price: {
...
f" {
    ...
    Fore.GREEN if trend == 'upward' else Fore.RED if trend == 'downward' else Fore.YELLOW
} {
    trend
}"
)
...
f" {
    ...
    Fore.RED if momentum == 'overbought' else Fore.GREEN if momentum == 'oversold' else Fore.YELLOW
} {
    momentum
}"
)
...
f" {
    ...
    Fore.GREEN if volume == 'high' else Fore.RED
} {
    volume
}"
)
...
self.logger.info(f" {
    Fore.YELLOW
}--- Dynamic Support/Resistance Clusters ---")
if "support_clusters" in self.levels:
self.logger.info(f" {
    Fore.CYAN
}Support Clusters: {
    ...
    ', '.join(map(str, self.levels['support_clusters']))}")
if "resistance_clusters" in self.levels:
self.logger.info(f" {
    Fore.CYAN
}Resistance Clusters: {
    ...
    ', '.join(map(str, self.levels['resistance_clusters']))}")
...
self.logger.info(f" {
    ...
    ', '.join([f'{label} at {value:.2f}' for label, value, vol in nearest_supports])}")
self.logger.info(f" {
    Fore.CYAN
}Nearest Resistances: {
    Fore.WHITE
} {
    ', '.join([f'{label} at {value:.2f}' for label, value, vol in nearest_resistances])}")
...
self.logger.info(f" {
    ...
    trade_suggestions['long'].get('entry', 'N/A')}")
self.logger.info(f" {
    Fore.GREEN
}Long Target: {
    trade_suggestions['long'].get('target', 'N/A')}")
else :
self.logger.info(f" {
    Fore.YELLOW
}No Long Trade Suggestions")

if trade_suggestions["short"]:
self.logger.info(f" {
    Fore.RED
}Short Entry: {
    trade_suggestions['short'].get('entry', 'N/A')}")
self.logger.info(f" {
    Fore.RED
}Short Target: {
    trade_suggestions['short'].get('target', 'N/A')}")
else :
self.logger.info(f" {
    Fore.YELLOW
}No Short Trade Suggestions")
self.logger.info(f" {
    Fore.YELLOW
}--- End Analysis ---")
...
logger.error(f"An error occurred during analysis: {
    e
}")
