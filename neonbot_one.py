        ...
        while retries < Constants.MAX_RETRIES:
            try:
                ...
                params["timestamp"] = str(
                    int(datetime.now(Constants.UTC_TZ).timestamp() * 1000)
                )
                params["sign"] = self._generate_signature(params)

                url = f"{self.config.base_url}{endpoint}"
                self.api_logger.debug(
                    f"API Request - Method: {method}, URL: {url}, Params: {params}"
                )
                ...
                try:
                    ...
                    self.logger.error(
                        f"Invalid JSON Response Received: {response.text}"
                    )
...
class TradingAnalyzer:
    ...
    def __init__(
        self, symbol: str, interval: str, logger: logging.Logger, bybit: Bybit
    ):
        self.symbol = symbol
    ...
    def determine_trend_momentum(
        self, indicator: pd.Series, overbought=70, oversold=30
    ) -> str:
        """Determines the trend based on momentum and volume indicators."""
        if indicator.empty:
            self.logger.warning(
                "Indicator series empty cannot determine momentum trend"
            )
            return "neutral"
    ...
    def determine_trend_volume(self, indicator: pd.Series) -> str:
        ...
        if indicator.empty:
            self.logger.warning(
                "Indicator series empty - cannot determine volume trend."
            )
            return "neutral"
...
class TradingAnalyzerEnhanced(TradingAnalyzer):
    ...
    def initialize_data(self):
        ...
        if self.df.empty:
            self.logger.error(
                f"Failed to fetch initial klines for {self.symbol} ({self.interval})."
            )
            return False
        self.indicators.df = (
            self.df
        )  # Pass DataFrame for technical indicator calculations
        return True
    ...
    def update_dynamic_sensitivity(self):
        ...
        if len(self.df) > 0:
            self.dynamic_sensitivity = (
                Constants.CLUSTER_SENSITIVITY * atr / self.df["close"].iloc[-1]
            )
        else:
            self.logger.warning(
                "DataFrame is empty. Cannot update dynamic sensitivity."
            )

    def identify_support_resistance_dynamic(
        self, window=Constants.SUPPORT_RESISTANCE_WINDOW
    ):
    ...
    def analyze(self, current_price: float):
        ...
        trend_wma = self.determine_trend_ma(
            short_window=wma_ind.iloc[-1], long_window=ema_ind.iloc[-1]
        )
        ...
        )  # Make sure to convert non standard types to string
