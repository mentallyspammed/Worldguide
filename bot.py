...
self.logger.info(f"BybitAPI initialized. Base URL: {
    base_url
}, WebSocket URL: {
    ws_url
}")
...
self.logger.exception(f"Request error during {
    method
} {
    full_url
}: {
...
raise BybitAPIError(f"Max retries exceeded for {
    full_url
}, request failed: {
    e
}") from e
except Exception as e:
self.logger.exception(f"Unexpected error during request: {
    e
}")
if attempt == self.retry_count - 1:
raise BybitAPIError(f"Max retries exceeded, unexpected error: {
    e
}") from e

async def get_symbol_info(self, symbol: str) -> Dict:
params = {
    "category": CATEGORY_LINEAR, "symbol": symbol
}
try:
...
self.logger.exception(f"Error getting symbol info: {
    e
}")
return None
...
self.logger.error(f"Error in get_current_price response: {
    response
}")
return None
except Exception as e:
self.logger.exception(f"get_current_price failed: {
    e
}")
return None
...
params = {
    "category": CATEGORY_LINEAR, "symbol": symbol, "interval": interval, "limit": limit
}
try:
data = await self._request("GET", "/v5/market/kline", params = params)
return data.get('result', {}).get('list')
except Exception as e:
self.logger.exception(f"Error getting klines: {
    e
}")
return None
...
self.logger.error(f"Unexpected response format from get_balance: {
    response
}")
return None
except Exception as e:
self.logger.exception(f"get_balance request failed: {
    e
}")
return None
...
self.logger.exception(f"Error placing order: {
    e
}")
return None
...
params = {
    "category": category, "symbol": symbol, "orderFilter": ORDER_TYPE_MARKET
} # Adjusted to fetch stop-loss orders
try:
...
self.logger.info(f"No active stop-loss order found for {
    symbol
}")
return None
else :
self.logger.error(f"Failed to fetch stop-loss orders: {
    response
}")
return None
except Exception as e:
self.logger.exception(f"Error fetching stop-loss orders: {
    e
}")
return None
...
self.logger.error(f"Failed to cancel order: {
    response
}")
return None
except Exception as e:
self.logger.exception(f"Error cancelling order: {
    e
}")
return None
...
if response and response.get('retCode') == 0:
self.logger.info(f"Successfully fetched position data for {
    symbol
}")
return response
else :
...
self.logger.exception(f"Error fetching position data: {
    e
}")
return None
...
self.logger.error(f"Invalid JSON format in: {
    config_file
}")
raise ConfigurationError(f"Invalid JSON format in: {
    config_file
}")
...
self.logger.info(f"Logging configured. Level: {
    log_level
}, File: {
    log_filename
}")
...
if not symbol_info:
self.logger.error(f"Could not fetch information for {
    symbol
}. Skipping.")
continue
...
self.logger.warning(
f"Trade quantity for {
    symbol
} was below minimum. Adjusted to: {
    min_trade_qty
}"
)
...
self.logger.exception(f"Error configuring parameters for {
    symbol
}: {
    e
}")
...
self.logger.info(f"WebSocket connected for {
    symbol
}.")
...
self.logger.exception(f"Websocket error for {
    symbol
}: {
    ws_exception
}")
...
self.logger.warning(f"Attempting reconnection for {
    symbol
} in {
    delay
} seconds, attempt {
    ...
    self.max_websocket_reconnect_attempts
}")
...
self.logger.error(f"Error closing WebSocket: {
    e
}")

if retries == self.max_websocket_reconnect_attempts:
self.logger.error(f"Max reconnection attempts reached for {
    symbol
}.")
break
if not self.running:
self.logger.info(f"Websocket handler exiting for {
    symbol
}.")
break

async def process_trade_data(self, trade_data: Dict):
self.logger.debug(f"Raw Websocket Data: {
    trade_data
}")
try:
...
self.logger.debug(f"Trade data received, symbol: {
    symbol
}")
for symbol_config in self.symbols:
...
self.logger.exception(f"Error in trade processing loop: {
    e
}")
...
self.logger.error(f"Symbol configuration not found for {
    symbol
}")
return
...
if not klines:
self.logger.error(f"No kline data received for {
    symbol
}")
return
...
self.logger.error(f"Missing closing prices in klines for {
    symbol
}")
return
...
self.logger.warning(f"Current price not available for {
    symbol
}")
return
...
self.logger.info(f"Trading disabled for {
    symbol
}.")

except Exception as e:
self.logger.exception(f"Error in trading strategy for {
    symbol
}: {
    e
}")
...
self.logger.error(f"Invalid side: {
    side
}")
return
...
self.logger.info(f"No existing stop loss order, creating a new one at {
    new_stop_loss_price
}")
...
self.logger.info(f"Updating stop loss order. New price: {
    new_stop_loss_price
}, Old price: {
    existing_sl_price
}")
...
self.logger.error(f"Failed to cancel old stop loss order {
    existing_order_id
}: {
    cancel_response
}")
...
self.logger.info(f"No active position found for symbol {
    symbol
} to manage trailing stop.")
else :
self.logger.error(f"Error fetching position data for {
    symbol
} to manage trailing stop: {
    position
}")
...
self.logger.error(f"Could not place {
    side
} Limit Order at {
    level:.4f
}. Response: {
...
self.logger.exception(f"Error placing grid order for {
    symbol
} at level {
    level
}: {
...
"""
        ...
        """
try:
self.logger.info(f"Executing trade logic: {
    side
} {
    qty
} {
...
self.logger.exception(f"Error executing trade logic: {
    e
}")
...
bot.logger.error(f"An unexpected error occurred: {
    e
}")
