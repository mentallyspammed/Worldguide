        ...
        while retries < Constants.MAX_RETRIES:
            try:
                ...
                if json_response.get("retCode", -1) != 0:
                    self.logger.error(
                        f"Bybit API returned non-zero retCode: {json_response}"
                    )
                ...
                if response.status_code in Constants.RETRY_ERROR_CODES:
                    ...
                    self.logger.error(
                        f"Bybit API HTTP error {response.status_code}: {e}"
                    )
...
class TradingAnalyzerEnhanced(TradingAnalyzer):
    ...
    def analyze(self, current_price: float):
        ...
        self.logger.info(
            f"  Aroon Up: {aroon_up.iloc[-1]:.2f}, Aroon Down: {aroon_down.iloc[-1]:.2f}"
        )
