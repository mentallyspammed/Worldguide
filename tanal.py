...
class TradingAnalyzer:
    ...
    def calculate_fibonacci_retracement(self, high: float, low: float, current_price: float) -> Dict[str, float]:
        ...
        try:
            ...
            self.fib_levels = fib_levels  # Update self.fib_levels with calculated levels
    ...
    def calculate_ema(self, window: int) -> pd.Series:
        ...
        try:
            ...
            self.logger.error(Fore.RED + "Missing required 'close' column to calculate EMA")
            return pd.Series(dtype='float64')
    ...
    def determine_trend_momentum(self) -> dict:
        ...
        # Volume Confirmation
        volume_ma = self.df["volume"].rolling(window=dynamic_params["volume_ma_period"]).mean()
        volume_confirmation = self.df["volume"].iloc[-1] > volume_ma.iloc[-1] * 1.25  # Increased sensitivity
        ...
        is_sideways = abs(self.df["high"].iloc[-1] - self.df["low"].iloc[-1]) < atr_range
        ...
        elif bullish_score > bearish_score and bullish_score >= trend_strength_threshold:
            return {"trend": "Uptrend", "strength": bullish_score}
        elif bearish_score > bullish_score and bearish_score >= trend_strength_threshold:
    ...
    def predict_next_level(self, current_price:float, nearest_supports: List[Tuple[str,float]], nearest_resistances: List[Tuple[str,float]]) -> str:
        ...
        if abs(closest_support[1]- current_price) < abs(closest_resistance[1] - current_price):
    ...
    def analyze(self, current_price: float):
        ...
        self.logger.info(f"{Fore.YELLOW}Fibonacci Levels (Support/Resistance):{Style.RESET_ALL}")
