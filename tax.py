...
class TradingAnalyzer:
    ...
    def find_nearest_levels(self, current_price: float) -> str:
        ...
        if next_resistance is None or (
            next_support is not None
            and abs(current_price - next_support) < abs(current_price - next_resistance)
        ):
...
if __name__ == "__main__":
    ...
    interval = input(
        f"{Fore.CYAN}Enter timeframe (e.g., 1, 5, 15, 60, 240, D): {Style.RESET_ALL}"
    )
    ...
    while True:
        try:
            ...
            if df.empty:
                logger.error(
                    f"{Fore.RED}Failed to fetch data for {symbol}.{Style.RESET_ALL}"
                )
                continue
            ...
            if current_price is None:
                logger.error(
                    f"{Fore.RED}Could not fetch the current price.{Style.RESET_ALL}"
                )
                continue
