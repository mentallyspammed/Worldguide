...
class TradingAnalyzer:
    ...
    def _add_technical_indicators(self):
        ...
        try:
            ...
            self.df["Stoch_RSI"] = StochasticOscillator(self.df["high"], self.df["low"], self.df["close"], window=STOCH_WINDOW, smooth_window=STOCH_SMOOTH_WINDOW).stoch()
    ...
    def calculate_supertrend(self, df, atr_period=10, multiplier=3):
        ...
        df['supertrend'] = np.where((df['in_uptrend'] == True), df['lowerband'], df['upperband'])
        return df
    ...
    def determine_trend(self) -> str:
        ...
        if self.df["close"].iloc[-1] > self.df["SMA_9"].iloc[-1] and self.df['ADX'].iloc[-1] > ADX_THRESHOLD:
          if self.df['HMA_fast'].iloc[-1] > self.df['WMA_slow'].iloc[-1]:
            return "Strong Uptrend"
          return "Uptrend"
        elif self.df["close"].iloc[-1] < self.df["SMA_9"].iloc[-1] and self.df['ADX'].iloc[-1] > ADX_THRESHOLD:
          if self.df['HMA_fast'].iloc[-1] < self.df['WMA_slow'].iloc[-1]:
    ...
    def _format_signal_output(self, signal: Dict[str, Any]) -> None:
        ...
        if signal['position'] != "NONE":
            ...
            print(f"Signal Strength: {Fore.YELLOW}{signal['strength']}%{Style.RESET_ALL}")
