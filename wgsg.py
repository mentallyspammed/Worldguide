...
class TradingAnalyzer:
    ...
    def _add_technical_indicators(self, df: pd.DataFrame):
        ...
        df["CMF"] = chaikin_money_flow(df['high'], df['low'], df['close'], df['volume']) # Corrected
    ...
    def determine_entry_signal(self, current_price:float) -> str | None:
        ...
        if trend == "bullish" and current_rsi < RSI_OVERSOLD and any(abs(current_price - support) < 0.02 * current_price for support in supports):
             return "long"
        elif trend == "bearish" and current_rsi > RSI_OVERBOUGHT and any(abs(current_price - resistance) < 0.02 * current_price for resistance in resistances):
             return "short"
        else:
            return "neutral"
