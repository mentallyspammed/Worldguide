...
class TradingOutput:
    ...
    def display_analysis(self, results: Dict):
        ...
        self._log_rsi_analysis(results.get("rsi_analysis", "N/A"), results.get("rsi_value", "N/A"))
        self._log_macd_analysis(results.get("macd_analysis", "N/A"))
        self._log_aroon_analysis(results.get("aroon_analysis", "N/A"))
        self._log_support_resistance(results.get("nearest_supports"), results.get("nearest_resistances"), current_price)
        self._log_fibonacci_levels(results.get("fibonacci_levels", {}))
        self._log_trade_suggestions(results.get("trade_suggestions", []))
        self._log_scores(results.get("long_score", "N/A"), results.get("short_score", "N/A"))
        self.logger.info(Style.RESET_ALL)

    def _log_trend_analysis(self, trend: str):
        self.logger.info(f" {Fore.YELLOW}Trend:{Style.RESET_ALL} {self._colorize_trend(trend)}")

    def _log_momentum_analysis(self, results: Dict):
        momentum = results.get("momentum", "N/A")
        self.logger.info(f" {Fore.YELLOW}Momentum:{Style.RESET_ALL} {self._colorize_momentum(momentum)}")

    def _log_volume_analysis(self, volume_analysis: str):
        self.logger.info(f" {Fore.YELLOW}Volume:{Style.RESET_ALL} {self._colorize_volume_analysis(volume_analysis)}")

    def _log_rsi_analysis(self, rsi_analysis: str, rsi_value: float):
        self.logger.info(f" {Fore.YELLOW}RSI:{Style.RESET_ALL} {self._colorize_rsi_analysis(rsi_analysis)} ({Fore.CYAN}{rsi_value:.2f}{Style.RESET_ALL})")

    def _log_macd_analysis(self, macd_analysis: str):
        self.logger.info(f" {Fore.YELLOW}MACD:{Style.RESET_ALL} {self._colorize_macd_analysis(macd_analysis)}")

    def _log_aroon_analysis(self, aroon_analysis: str):
        self.logger.info(f" {Fore.YELLOW}Aroon:{Style.RESET_ALL} {self._colorize_aroon_analysis(aroon_analysis)}")

    def _log_support_resistance(self, supports: Optional[float], resistances: Optional[float], current_price: float):
        self.logger.info(f" {Fore.YELLOW}Support/Resistance:{Style.RESET_ALL}")
        if supports is not None:
            distance = current_price - supports
            self.logger.info(f"  {Fore.GREEN}Nearest Support:{Style.RESET_ALL} {Fore.CYAN}{supports:.4f}{Style.RESET_ALL} (Distance: {Fore.GREEN}{distance:.4f}{Style.RESET_ALL})")
        else:
            self.logger.info(f"  {Fore.GREEN}Nearest Support:{Style.RESET_ALL} {Fore.YELLOW}None Detected{Style.RESET_ALL}")

        if resistances is not None:
            distance = current_price - resistances
            self.logger.info(f"  {Fore.RED}Nearest Resistance:{Style.RESET_ALL} {Fore.CYAN}{resistances:.4f}{Style.RESET_ALL} (Distance: {Fore.RED}{distance:.4f}{Style.RESET_ALL})")
        else:
            self.logger.info(f"  {Fore.RED}Nearest Resistance:{Style.RESET_ALL} {Fore.YELLOW}None Detected{Style.RESET_ALL}")
        ...
        if fib_levels:
            for level_name, levels in fib_levels.items():
                if isinstance(levels, dict):
                    self.logger.info(f"  {Fore.MAGENTA}{level_name.capitalize()}:{Style.RESET_ALL}")
                    for level, value in levels.items():
                        self.logger.info(f"    - {level}: {Fore.CYAN}{value:.4f}{Style.RESET_ALL}")
        else:
            self.logger.info(f"   {Fore.YELLOW}No Fibonacci pivot levels calculated.{Style.RESET_ALL}")
    ...
    def _log_trade_suggestions(self, trade_suggestions: List[Dict]):
        ...
        if trade_suggestions:
            for suggestion in trade_suggestions:
                ...
                if signal == "long":
                    self.logger.info(f"   {Fore.GREEN}Long Signal:{Style.RESET_ALL} Confidence: {Fore.CYAN}{confidence:.2f}{Style.RESET_ALL}")
                elif signal == "short":
                    self.logger.info(f"   {Fore.RED}Short Signal:{Style.RESET_ALL} Confidence: {Fore.CYAN}{confidence:.2f}{Style.RESET_ALL}")
        else:
            self.logger.info(f"   {Fore.YELLOW}No trade suggestions at this time.{Style.RESET_ALL}")

    def _log_scores(self, long_score: float, short_score: float):
        self.logger.info(f" {Fore.YELLOW}Scores:{Style.RESET_ALL} Long: {Fore.GREEN}{long_score:.2f}{Style.RESET_ALL}, Short: {Fore.RED}{short_score:.2f}{Style.RESET_ALL}")
