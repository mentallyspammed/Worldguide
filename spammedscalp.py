...
from ta.trend import MACD, EMAIndicator, HullMovingAverage, ADXIndicator
...
class TradingAnalyzer:
    ...
    def _add_technical_indicators(self):
        ...
        self.df["HMA_fast"] = HullMovingAverage(self.df["close"], window=FAST_HMA_WINDOW).hma_indicator()
    ...
    def determine_trend(self) -> str:
        ...
        if current_adx < ADX_THRESHOLD:
            return "neutral"
        fast_hma, slow_wma = self.df["HMA_fast"].iloc[-1], self.df["WMA_slow"].iloc[-1]
        if fast_hma > slow_wma:
            return "bullish"
        if fast_hma < slow_wma:
            return "bearish"
        return "neutral"
    ...
    def determine_entry_signal(self, current_price: float, supports, resistances) -> Optional[str]:
        ...
        score = 0
        score += 1 if fast_hma > slow_wma else -1
        score += 1 if stoch_rsi < rsi_oversold else -1
        score += 1 if volume_spike else 0
        score += 1 if supertrend == self.df['upperband'].iloc[-1] else -1

        if score > 1 and (near_fib or near_support or near_resistance):
            return "long"
    ...
    def find_hidden_bullish_divergence(self):
        """Identify hidden bullish divergence in the MACD histogram."""
        divergences = []
        for i in range(1, len(self.df)):
            if self.df['close'].iloc[i] > self.df['close'].iloc[i-1] and self.df['MACD'].iloc[i] < self.df['MACD'].iloc[i-1]:
    ...
    def analyze(self, current_price: float):
        ...
        if entry_signal: self.logger.info(f"{neon_green}Trade Signal: {entry_signal}{Style.RESET_ALL}")
        else:
